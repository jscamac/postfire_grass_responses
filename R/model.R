...
run_stan_model <- function(data, sim_pretillers=NULL, sim_ldmc = NULL) {
  ...
  stan_data <- list(
    ...
    sim_pretillers = (sim_pretillers - mn_pretillers) / (2 * sd_pretillers), # CS TPRE values for predictions
    sim_ldmc = (sim_ldmc - mn_ldmc)/ (2 * sd_ldmc), # CS LDMC values for predictions
...
data{
  ...
  real sim_pretillers[sim_nobs_pretillers]; 
  real sim_ldmc[sim_nobs_ldmc]; 
}
parameters{ 
...
transformed parameters { 
  ...
  for (i in 1:n_obs) {
    count[i] = exp(b_tribe[tribe[i]] + b_spp[spp[i]] + b_trt * treatment[i] + b_ldmc * ldmc[i] + b_c4 * c4[i] + b_tpre * tillers_prefire[i]);
    p_death[i] = inv_logit(a_tribe[tribe[i]] + a_spp[spp[i]] + a_trt * treatment[i] + a_ldmc * ldmc[i] + a_c4 * c4[i] + a_tpre * tillers_prefire[i]);
  }
}
model { 
  a_tribe_mu ~ cauchy(0,5);
  a_tribe_sd ~ cauchy(0,5);
  a_tribe ~ normal(a_tribe_mu, a_tribe_sd);
  ...
  b_tribe ~ normal(b_tribe_mu, b_tribe_sd);
...
generated quantities { 
  ...
  # Partial dependencies TPRE
  for (j in 1:sim_nobs_pretillers) {
    pred_p_death_c3_tpre[j] = inv_logit(a_tribe_mu + a_tpre * sim_pretillers[j]);
    pred_p_death_c4_tpre[j] = inv_logit(a_tribe_mu + a_tpre * sim_pretillers[j] + a_c4);
    pred_count_c3_tpre[j] = exp(b_tribe_mu + b_tpre * sim_pretillers[j]);
    pred_count_c4_tpre[j] = exp(b_tribe_mu + b_tpre * sim_pretillers[j] + b_c4);
  }
  
  # Partial dependencies LDMC
  for (k in 1:sim_nobs_ldmc) {
    pred_p_death_c3_ldmc[k] = inv_logit(a_tribe_mu + a_ldmc * sim_ldmc[k]);
    pred_p_death_c4_ldmc[k] = inv_logit(a_tribe_mu + a_ldmc * sim_ldmc[k] + a_c4);
    pred_count_c3_ldmc[k] = exp(b_tribe_mu + b_ldmc * sim_ldmc[k]);
    pred_count_c4_ldmc[k] = exp(b_tribe_mu + b_ldmc * sim_ldmc[k] + b_c4);
    }
}"
